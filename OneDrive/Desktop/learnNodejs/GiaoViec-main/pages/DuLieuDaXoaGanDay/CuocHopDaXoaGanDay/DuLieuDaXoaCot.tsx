...
const App: React.FC = () => {
  ...
  const expandable = {
    ...
    expandIcon: ({ expanded, onExpand, record }: {
      expanded: any, onExpand: any, record: any,
    }) => (
    ...
    expandedRowRender: (record: any) => (
      <div onClick={() => handleSelectRow(record.key)} style={{ display: "flex" }}>
        <p>
          {" "}
          <Image width={100}
            height={25}
            alt='' src={record.img} style={{ paddingLeft: "70px" }} />
        </p>
        <p style={{ fontWeight: "500", paddingLeft: "60px" }}>{record.name}</p>
        <p style={{ fontWeight: "500", paddingLeft: "30px" }}>{record.diadiem}</p>
